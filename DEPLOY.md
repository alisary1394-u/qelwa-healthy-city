...
- **استمرارية البيانات (مهم جداً):** عند كل نشر (تحديث) تُشغّل Railway حاوية جديدة. **بدون تخزين دائم تُحذف كل البيانات.** يجب إضافة **Volume** وربطه بالمسار **`/data`** للخدمة (من Railway → الخدمة → **Variables** أو **Volumes** → إضافة Volume و mount path: `/data`). قاعدة البيانات في `/data/qelwa.db` والنسخ الاحتياطية في `/data/backups` — بدون Volume يختفي كل ذلك بعد كل نشر. راجع `docs/DATA_LOSS_ON_UPDATE_ANALYSIS.md` لتحليل مفصل.
