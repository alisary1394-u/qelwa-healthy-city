...
- **هل هو خطأ؟** في سياق النشر المتتابع يعتبر طبيعياً. التطبيق يغلق الاستماع ثم يخرج برمز 0. لتقليل ظهور رسالة "npm error signal SIGTERM" استخدم أمر تشغيل مباشر: **Start Command** = `node server/index.js` (بدون `npm start`) — انظر `railway.json` و `DEPLOY.md`.
...
### ظهور "CRASHED" ورسالة تحذير بالبريد أثناء النشر (بعد ربط Volume)

- **ما يحدث:** عند النشر مع Volume، Railway يوقف الحاوية القديمة أولاً (لتفادي تشغيل نسختين على نفس الـ Volume)، ثم يشغّل الحاوية الجديدة. خلال هذه اللحظة قد تظهر حالة **CRASHED** للحاوية القديمة أو يفشل فحص الصحة للحاوية الجديدة لثوانٍ، ثم يعود النشر إلى الوضع الطبيعي وتصل رسالة تحذير بالبريد.
- **هل هو خطأ حقيقي؟** في الغالب لا — التطبيق يعود للعمل بعد ثوانٍ والنشر ينجح. التحذير ناتج عن فترة الانتقال أثناء استبدال الحاوية.
- **ما يمكن فعله لتقليل التحذير:**
  1. **مهلة فحص الصحة:** في Railway → الخدمة → **Variables** أضف إن وُجد: `RAILWAY_HEALTHCHECK_TIMEOUT_SEC=120` (أو قيمة أكبر) حتى لا يُعتبر النشر فاشلاً في أول ثوانٍ.
  2. **إعدادات الإشعارات:** في Railway تحقق من إعدادات الإشعارات (Notifications) — قد يمكن تعطيل تنبيه "Deployment failed" أو تقييده بحالات الفشل بعد انتهاء المهلة فقط.
  3. **التأكد من المسار:** تأكد أن **Health Check Path** مضبوط على `/api/health` (وليس `/health`) لأن التطبيق يرد على هذا المسار.
