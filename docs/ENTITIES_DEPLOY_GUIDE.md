...
تم إنشاء تعريفات جميع الكيانات في مجلد `backend-platform/entities/` بصيغة JSON Schema. لتفعيلها على مشروعك في Base44 اتبع الخطوات التالية.
...
ستُطلب منك تسجيل الدخول واختيار التطبيق (أو إنشاء تطبيق جديد). بعد الربط يُنشأ ملف `backend-platform/.app.jsonc` يحتوي معرف التطبيق (لا ترفعه إلى Git).
...
سيتم مزامنة جميع الملفات في `backend-platform/entities/*.json` مع قاعدة بيانات تطبيقك في Base44. كل كيان سيُنشأ أو يُحدَّث حسب التعريف المحلي.
...
1. عدّل الملف المناسب في `backend-platform/entities/`.
